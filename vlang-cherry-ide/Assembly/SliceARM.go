...
func (sp *SliceProcessor) DeclararSlice(nombre, tipoElemento string, elementos []*ResultadoExpresion) error {
	...
	// AJUSTAR STACK SI ES EL PRIMER SLICE
	...
	if tamaño == 0 {
		...
		//Actualizar offset para próximo slice
		sp.offsetStack += (tamaño * 8) // Incrementar offset positivo
	}
...
func (sp *SliceProcessor) AlmacenarElementoLiteral(elemento *ResultadoExpresion, offset int) {
	...
	case "int":
		...
		sp.armGen.Instructions = append(sp.armGen.Instructions,
			...
			fmt.Sprintf("str x0, [sp, #%d]", offset)) // AHORA ES OFFSET POSITIVO
	...
	case "float":
		...
		sp.armGen.Instructions = append(sp.armGen.Instructions,
			fmt.Sprintf("str %s, [sp, #%d]", regFloat, offset))
	...
	case "string":
		...
		sp.armGen.Instructions = append(sp.armGen.Instructions,
			...
			fmt.Sprintf("str x0, [sp, #%d]", offset))
...
func (sp *SliceProcessor) AlmacenarElementoRegistro(elemento *ResultadoExpresion, offset int) {
	...
	case "int", "bool":
		sp.armGen.Instructions = append(sp.armGen.Instructions,
			fmt.Sprintf("str %s, [sp, #%d]", elemento.Registro, offset))
...
func (sp *SliceProcessor) AccederElemento(nombre string, indice int) (*ResultadoExpresion, error) {
	...
	// Cargar elemento desde stack
	sp.armGen.Instructions = append(sp.armGen.Instructions,
		fmt.Sprintf("ldr %s, [sp, #%d]    // cargar elemento %d",
...
func (sp *SliceProcessor) ImprimirElementoEnIndice(sliceInfo *SliceInfo, indice int) {
	...
	case "int":
		...
		sp.armGen.Instructions = append(sp.armGen.Instructions,
			fmt.Sprintf("ldr x0, [sp, #%d]    // cargar int[%d]", offsetElemento, indice),
			"bl print_int")

	case "float":
		sp.armGen.UsarFuncion("print_float")
		sp.armGen.Instructions = append(sp.armGen.Instructions,
			fmt.Sprintf("ldr d0, [sp, #%d]    // cargar float[%d]", offsetElemento, indice),
			"bl print_float")

	case "string":
		sp.armGen.UsarFuncion("print_string")
		sp.armGen.Instructions = append(sp.armGen.Instructions,
			fmt.Sprintf("ldr x0, [sp, #%d]    // cargar string[%d]", offsetElemento, indice),
			"bl print_string")

	case "bool":
		sp.armGen.UsarFuncion("print_bool")
		sp.armGen.Instructions = append(sp.armGen.Instructions,
			fmt.Sprintf("ldr x0, [sp, #%d]    // cargar bool[%d]", offsetElemento, indice),
			"bl print_bool")
	}
}
...
func (sp *SliceProcessor) generarCodigoBusqueda(sliceInfo *SliceInfo, valorBuscado *ResultadoExpresion) (*ResultadoExpresion, error) {
	...
	sp.armGen.Instructions = append(sp.armGen.Instructions,
		fmt.Sprintf("add %s, sp, #%d", regElemento, offsetBase),                      // Base del slice
...
func (sp *SliceProcessor) Join(nombreSlice string, separador *ResultadoExpresion) (*ResultadoExpresion, error) {
    ...
    if sliceInfo.Tamaño == 1 {
        sp.armGen.Comment("Solo un elemento, retornándolo directamente")
        registroResultado := sp.expresionesProcessor.NuevoRegistroTmp()
        offsetPrimerElemento := sliceInfo.OffsetStack
        sp.armGen.Instructions = append(sp.armGen.Instructions,
            fmt.Sprintf("ldr %s, [sp, #%d]", registroResultado, offsetPrimerElemento))
        return &ResultadoExpresion{
            Registro:  registroResultado,
            Tipo:      "string",
            EsLiteral: false,
...
func (sp *SliceProcessor) generarCodigoJoin(sliceInfo *SliceInfo, separador *ResultadoExpresion) (*ResultadoExpresion, error) {
    // Etiquetas para control de flujo
    labelLoop := fmt.Sprintf("join_loop_%s_%d", sliceInfo.Nombre, sp.expresionesProcessor.GetContadorEtiqueta())
    labelEnd := fmt.Sprintf("join_end_%s_%d", sliceInfo.Nombre, sp.expresionesProcessor.GetContadorEtiqueta())

    //  CORREGIR: Usar registros diferentes para evitar conflictos
    regIndice := "x9"           // Índice actual 
    regElemento := "x10"        // Elemento actual del slice
    regSeparador := "x11"       // Separador
    regResultado := "x12"       // String resultado acumulado
    regTempBuffer := "x13"      // Buffer temporal para concatenación
    regTemp := "x14"            // Registro temporal para intercambios

    sp.armGen.Comment("Inicializar registros para join")

    // Cargar separador en registro
    if separador.EsLiteral {
        etiquetaSep := sp.expresionesProcessor.AgregarMensajeString(separador.Valor.(string))
        sp.armGen.Instructions = append(sp.armGen.Instructions,
    ...
    sp.armGen.Instructions = append(sp.armGen.Instructions,
        fmt.Sprintf("adr %s, buffer_string", regResultado),
        fmt.Sprintf("adr %s, temp_buffer", regTempBuffer))

    // Inicializar con el primer elemento (índice 0)
    offsetPrimerElemento := sliceInfo.OffsetStack
    sp.armGen.Instructions = append(sp.armGen.Instructions,
        fmt.Sprintf("ldr %s, [sp, #%d]", regElemento, offsetPrimerElemento))

    // Copiar primer elemento al buffer resultado
    sp.armGen.UsarFuncion("concat_strings")
    etiquetaVacia := sp.expresionesProcessor.AgregarMensajeString("")
    sp.armGen.Instructions = append(sp.armGen.Instructions,
        fmt.Sprintf("mov x0, %s", regElemento),          // src1 (primer elemento)
        fmt.Sprintf("adr x1, %s", etiquetaVacia),        // src2 (string vacío)
        fmt.Sprintf("mov x2, %s", regResultado),         // dest
        "bl concat_strings")

    // Inicializar índice en 1 (empezar desde el segundo elemento)
    sp.armGen.Instructions = append(sp.armGen.Instructions,
    ...
    // Concatenar separador al resultado
    sp.armGen.Instructions = append(sp.armGen.Instructions,
        fmt.Sprintf("mov x0, %s", regResultado),         // src1 (resultado actual)
        fmt.Sprintf("mov x1, %s", regSeparador),         // src2 (separador)
        fmt.Sprintf("mov x2, %s", regTempBuffer),        // dest temporal
        "bl concat_strings")

    //  CORREGIR: Intercambiar buffers SIN tocar el índice
    sp.armGen.Instructions = append(sp.armGen.Instructions,
        fmt.Sprintf("mov %s, %s", regTemp, regResultado),      // temp = resultado
        fmt.Sprintf("mov %s, %s", regResultado, regTempBuffer), // resultado = temp_buffer
        fmt.Sprintf("mov %s, %s", regTempBuffer, regTemp))      // temp_buffer = temp

    // Cargar elemento actual del slice - USAR REGISTRO TEMPORAL
    offsetBase := sliceInfo.OffsetStack
    regElementoAddr := "x15" //  NUEVO: Registro para dirección del elemento
    sp.armGen.Instructions = append(sp.armGen.Instructions,
        fmt.Sprintf("add %s, sp, #%d", regElementoAddr, offsetBase),
        fmt.Sprintf("ldr %s, [%s, %s, lsl #3]", regElemento, regElementoAddr, regIndice))

    // Concatenar elemento actual al resultado
    sp.armGen.Instructions = append(sp.armGen.Instructions,
        fmt.Sprintf("mov x0, %s", regResultado),         // src1 (resultado + separador)
        fmt.Sprintf("mov x1, %s", regElemento),          // src2 (elemento actual)
        fmt.Sprintf("mov x2, %s", regTempBuffer),        // dest temporal
        "bl concat_strings")

    //  CORREGIR: Intercambiar buffers otra vez SIN tocar el índice
    sp.armGen.Instructions = append(sp.armGen.Instructions,
        fmt.Sprintf("mov %s, %s", regTemp, regResultado),      // temp = resultado
        fmt.Sprintf("mov %s, %s", regResultado, regTempBuffer), // resultado = temp_buffer
        fmt.Sprintf("mov %s, %s", regTempBuffer, regTemp))      // temp_buffer = temp

    //  CRÍTICO: Incrementar índice y continuar - MANTENER x9 COMO ÍNDICE
    sp.armGen.Instructions = append(sp.armGen.Instructions,
        fmt.Sprintf("add %s, %s, #1", regIndice, regIndice),  // x9 = x9 + 1
        fmt.Sprintf("b %s", labelLoop))
    ...
    sp.armGen.Comment(fmt.Sprintf("join completado, resultado en %s", regResultado))
...
func (sp *SliceProcessor) copiarElementosExistentes(sliceInfo *SliceInfo, nuevoOffset int) {
    ...
    for i := 0; i < sliceInfo.Tamaño; i++ {
        ...
        sp.armGen.Instructions = append(sp.armGen.Instructions,
            fmt.Sprintf("ldr %s, [sp, #%d]    // cargar elemento %d original", regElemento, offsetOrigen, i),
            fmt.Sprintf("str %s, [sp, #%d]    // guardar elemento %d en nueva posición", regElemento, offsetDestino, i))
    }
}
...
func (sp *SliceProcessor) generarAccesoDirecto(sliceInfo *SliceInfo, indice int) (*ResultadoExpresion, error) {
    ...
    case "int", "bool":
        registroResultado := sp.expresionesProcessor.NuevoRegistroTmp()
        sp.armGen.Instructions = append(sp.armGen.Instructions,
            fmt.Sprintf("ldr %s, [sp, #%d]    // cargar %s[%d]", registroResultado, offsetElemento, sliceInfo.Nombre, indice))
        return &ResultadoExpresion{
            Registro:  registroResultado,
            Tipo:      sliceInfo.TipoElemento,
            EsLiteral: false,
    ...
    case "float":
        regFloat := sp.expresionesProcessor.NuevoRegistroFloatTmp()
        sp.armGen.Instructions = append(sp.armGen.Instructions,
            fmt.Sprintf("ldr %s, [sp, #%d]    // cargar float %s[%d]", regFloat, offsetElemento, sliceInfo.Nombre, indice))
        return &ResultadoExpresion{
            Registro:  regFloat,
            Tipo:      sliceInfo.TipoElemento,
            EsLiteral: false,
    ...
    case "string":
        registroResultado := sp.expresionesProcessor.NuevoRegistroTmp()
        sp.armGen.Instructions = append(sp.armGen.Instructions,
            fmt.Sprintf("ldr %s, [sp, #%d]    // cargar string %s[%d]", registroResultado, offsetElemento, sliceInfo.Nombre, indice))
        return &ResultadoExpresion{
            Registro:  registroResultado,
            Tipo:      sliceInfo.TipoElemento,
            EsLiteral: false,
...
func (sp *SliceProcessor) generarAccesoConVerificacion(sliceInfo *SliceInfo, indiceExpr *ResultadoExpresion) (*ResultadoExpresion, error) {
    ...
    case "int", "bool":
        sp.armGen.Instructions = append(sp.armGen.Instructions,
            fmt.Sprintf("add %s, sp, #%d", regElemento, offsetBase),
    ...
    if valorExpr.EsLiteral {
        ...
        case "int":
            ...
            sp.armGen.Instructions = append(sp.armGen.Instructions,
                ...
                fmt.Sprintf("str %s, [sp, #%d]    // %s[%d] = %d", regTmp, offsetElemento, sliceInfo.Nombre, indice, valor))
        case "bool":
            valor := 0
            ...
            sp.armGen.Instructions = append(sp.armGen.Instructions,
                fmt.Sprintf("mov %s, #%d", regTmp, valor),
                fmt.Sprintf("str %s, [sp, #%d]    // %s[%d] = %t", regTmp, offsetElemento, sliceInfo.Nombre, indice, valorExpr.Valor.(bool)))
        case "string":
            etiqueta := sp.expresionesProcessor.AgregarMensajeString(valorExpr.Valor.(string))
            regTmp := sp.expresionesProcessor.NuevoRegistroTmp()
            sp.armGen.Instructions = append(sp.armGen.Instructions,
                fmt.Sprintf("adr %s, %s", regTmp, etiqueta),
                fmt.Sprintf("str %s, [sp, #%d]    // %s[%d] = \"%s\"", regTmp, offsetElemento, sliceInfo.Nombre, indice, valorExpr.Valor.(string)))
        case "float":
            regFloat := sp.expresionesProcessor.NuevoRegistroFloatTmp()
            sp.expresionesProcessor.CargarConstanteFloat(regFloat, valorExpr.Valor.(float64))
            sp.armGen.Instructions = append(sp.armGen.Instructions,
                fmt.Sprintf("str %s, [sp, #%d]    // %s[%d] = %f", regFloat, offsetElemento, sliceInfo.Nombre, indice, valorExpr.Valor.(float64)))
        }
    } else {
        // Valor en registro: copiar directamente
        sp.armGen.Instructions = append(sp.armGen.Instructions,
            fmt.Sprintf("str %s, [sp, #%d]    // %s[%d] = registro", valorExpr.Registro, offsetElemento, sliceInfo.Nombre, indice))
    }
