...
func (vp *VariablesProcessor) DeclararVariable(nombre, tipo string, valor *ResultadoExpresion) error {
	vp.armGen.Comment(fmt.Sprintf("=== DECLARAR VARIABLE: %s %s ===", nombre, tipo))
	...
	// 2. Calcular offset: la nueva variable está en [sp] después de reservar espacio
	offset := vp.tablaSimbolos.StackActual * 8 // Cada variable ocupa 8 bytes
	...
	vp.armGen.Comment(fmt.Sprintf("Variable %s declarada en offset %d", nombre, offset))
	return nil
}
...
func (vp *VariablesProcessor) inicializarVariableInt(simbolo *Simbolo, valor *ResultadoExpresion) {
	// ✅ CORREGIDO: La variable recién declarada siempre está en [sp]
	direccion := "[sp]"
...
func (vp *VariablesProcessor) CargarVariable(nombre string, tipoDestino string) (*ResultadoExpresion, error) {
	...
	vp.armGen.Comment(fmt.Sprintf("=== CARGAR VARIABLE: %s ===", nombre))

	// ✅ CORREGIDO: Calcular dirección basada en la posición en el stack
...
func (vp *VariablesProcessor) AsignarVariable(nombre string, valor *ResultadoExpresion) error {
	...
	vp.armGen.Comment(fmt.Sprintf("=== ASIGNAR VARIABLE: %s = %s ===", nombre, valor.Tipo))
	...
	// ✅ CORREGIDO: Asignación a variable existente
	direccion := fmt.Sprintf("[sp, #%d]", simbolo.Offset)
	...
	case "float":
		...
		// Agregar casos para string y bool si es necesario
	}
...
func (vp *VariablesProcessor) sonTiposCompatibles(tipoVariable, tipoValor string) bool {
	...
	if tipoVariable == "float" && tipoValor == "int" {
		return true
	}
	return false
}
...
func (vp *VariablesProcessor) DeclararVariable(nombre, tipo string, valor *ResultadoExpresion) error {
	...
	vp.armGen.Comment(fmt.Sprintf("Variable %s declarada en [sp] (offset actual: %d)", nombre, offset))
	return nil
}
